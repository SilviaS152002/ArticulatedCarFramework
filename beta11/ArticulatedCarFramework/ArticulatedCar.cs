        ...
        public partial class ArticulatedCar : Car
        {
            ...
            public override Location PositionWheelBoundsFront(Location wheelBoundsF, Graph graph, MovementInfo info, bool update)
            {
                ...
                for (int i = 0; i < wheelsetPivots.Count; i++)
                {
                    ...
                    {
                        Graph.PositionRotation prF = graph.GetPositionRotation(graph.LocationByMoving(locations[i], wheelsetPivots[i].Definition.OffsetA), accuracy);
                        Graph.PositionRotation prR = graph.GetPositionRotation(graph.LocationByMoving(locations[i], wheelsetPivots[i].Definition.OffsetB), accuracy);
                    ...
                    {
                        wheelsetPivots[i].posRot = graph.GetPositionRotation(locations[i], accuracy);
                    }
