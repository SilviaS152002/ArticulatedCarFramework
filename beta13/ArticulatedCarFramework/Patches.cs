...
using UI.Tags;
using UnityEngine;
using static Core.Bezier.BezierDistanceParameterCache;
...
namespace NS15
{
    namespace ArticulatedCarFramework
    {
        ...
        public static class TrainControllerPatches
        {
            ...
            [HarmonyPatch("CheckForCarsAtPoint"), HarmonyPostfix]

            public static void CheckForCarsAtPointPostfix(TrainController __instance, HashSet<string> ____checkForCarKeys, Dictionary<string, Car> ____carLookup, Vector3 point, float radius, HashSet<Car> foundCars, Location? sameRouteRequirement = null)
            {
                foreach (string checkForCarKey in ____checkForCarKeys)
                {
                    if (!____carLookup.TryGetValue(checkForCarKey, out Car car))
                    {
                        Log.Error("CheckForCarAtPoint: AABB tree returned {carId} but car is not in _carLookup", checkForCarKey);
                        continue;
                    }
                    if (car is ArticulatedCar)
                    {
                        // Debug.Log($"CheckForCarsAtPointPostfix captured ArticulatedCar {car.name}");
                        (Graph.PositionRotation, float)[] rectangles = ((ArticulatedCar)car).GetRectCheck2(); // ((ArticulatedCar)car).GetRectCheck(__instance.graph);
                        foreach ((Graph.PositionRotation pr, float len) in rectangles)
                        {
                            float num = radius + len / 2f + 10f;
                            if (Vector3.SqrMagnitude(pr.Position - point) > num * num)
                            {
                                continue;
                            }
                            Vector3 dist = point - pr.Position;
                            if (RotatedRectContains(position: new Vector2(dist.z, dist.x), rectLong: len, rectShort: 2f, rotation: pr.Rotation, radius: radius))
                            {
                                // Debug.Log($"Hit: Arti {car.name} position: {new Vector2(dist.x, dist.z)}, rectLong: {len}, rectShort: {2f}, rotation: {pr.Rotation.eulerAngles}, radius: {radius}");
                                foundCars.Add(car);
                                break;
                            }
                        }
                    ...
                    {
                        Vector3 centerPosition = car.GetCenterPosition(__instance.graph);
                        float num = radius + car.carLength / 2f + 10f;
                        if (Vector3.SqrMagnitude(centerPosition - point) > num * num)
                        {
                            continue;
                        }
                        Quaternion centerRotationCheckForCar = car.GetCenterRotationCheckForCar(__instance.graph);
                        Vector3 vector = point - centerPosition;
                        if (RotatedRectContains(position: new Vector2(vector.x, vector.z), rectLong: car.carLength, rectShort: 2f, rotation: centerRotationCheckForCar, radius: radius))
                        {
                            Debug.Log($"Hit: Car  {car.name} position: {new Vector2(vector.x, vector.z)}, rectLong: {car.carLength}, rectShort: {2f}, rotation: {centerRotationCheckForCar.eulerAngles}, radius: {radius}");
                        }
                    }
                    */
                }
            ...
            public static void CheckForCarsAtPointPostfix(TrainController __instance, HashSet<string> ____checkForCarKeys, Vector3 point, float radius, HashSet<Car> foundCars, Location? sameRouteRequirement = null)
            {
                Debug.Log($"CheckForCarsAtPoint returned {foundCars.Count} ({string.Join(", ", foundCars.Select(item => item.name))}) of {____checkForCarKeys.Count} cars ({string.Join(", ", ____checkForCarKeys)})");
            }
