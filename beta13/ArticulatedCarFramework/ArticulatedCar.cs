...
namespace NS15
{
    namespace ArticulatedCarFramework
    {
        ...
        public class ArticulatedPivotDefinition
        {
            ...
            public bool AccurateTracking { get; set; }
        ...
        public partial class ArticulatedCar : Car
        {
            ...
            List<Location> PivotLocations = new List<Location>(); // For switch overlap check
            public (Graph.PositionRotation, float)[] GetRectCheck(Graph graph)
            {
                Vector3[] pos = PivotLocations.Select(pl => graph.GetPosition(pl)).ToArray();
                (Graph.PositionRotation pr, float length)[] result = new (Graph.PositionRotation, float)[pos.Length - 1];
                for (int i = 0; i < result.Length; i++)
                {
                    result[i] = (
                        new Graph.PositionRotation()
                        {
                            Position = (pos[i] + pos[i + 1]) / 2f,
                            Rotation = Quaternion.LookRotation(Vector3.Scale(pos[i] - pos[i + 1], new Vector3(1, 0, 1)))
                        },
                        (pos[i + 1] - pos[i]).magnitude + 2f
                    );
                }
                return result;
            }

            public (Graph.PositionRotation pr, float length)[] GetRectCheck2()
            {
                List<ArticulatedPivot> bodyPivots = Pivots.Where(p => !p.IsWheelset).ToList();

                if (!MainPivot.IsWheelset) { bodyPivots.Add(MainPivot); }

                bodyPivots.OrderByDescending(pivot => pivot.Definition.Position);

                return bodyPivots.Select(pivot => (pivot.posRot, Mathf.Abs(pivot.parentA.Definition.Position - pivot.parentB.Definition.Position) + 1f)).ToArray();
            }
            ...
            public override Location PositionWheelBoundsFront(Location wheelBoundsF, Graph graph, MovementInfo info, bool update)
            {
                ...
                wheelsetPivots.OrderByDescending(pivot => pivot.Definition.Position);

                float halfCarLength = carLength / 2f;

                PivotLocations = [graph.LocationByMoving(wheelBoundsF, wheelInsetF - halfCarLength + wheelsetPivots.First().Definition.Position)];

                for (int i = 1; i < wheelsetPivots.Count; i++)
                {
                        PivotLocations.Add(graph.LocationByMoving(PivotLocations[i - 1], wheelsetPivots[i].Definition.Position - wheelsetPivots[i - 1].Definition.Position));
                }

                Location wheelBoundsR = graph.LocationByMoving(PivotLocations.Last(), wheelInsetR - halfCarLength - wheelsetPivots.Last().Definition.Position);
                ...
                UpdateCurvatureForLocation(PivotLocations.Random());

                PositionAccuracy accuracy = (IsVisible ? PositionAccuracy.High : PositionAccuracy.Standard);

                _truckGizmoPosRots = new List<Graph.PositionRotation>();

                for (int i = 0; i < wheelsetPivots.Count; i++)
                {
                    if (wheelsetPivots[i].Definition.AccurateTracking)
                    {
                        Graph.PositionRotation prF = graph.GetPositionRotation(graph.LocationByMoving(PivotLocations[i], wheelsetPivots[i].Definition.OffsetA), accuracy);
                        Graph.PositionRotation prR = graph.GetPositionRotation(graph.LocationByMoving(PivotLocations[i], wheelsetPivots[i].Definition.OffsetB), accuracy);
                        float ratio = wheelsetPivots[i].Definition.OffsetA / (wheelsetPivots[i].Definition.OffsetA - wheelsetPivots[i].Definition.OffsetB);
                        wheelsetPivots[i].posRot = new Graph.PositionRotation
                        {
                            Position = Vector3.Lerp(prF.Position, prR.Position, ratio),
                            Rotation = Quaternion.LookRotation(prF.Position - prR.Position, Vector3.Lerp(prF.Rotation * Vector3.up, prR.Rotation * Vector3.up, .5f))
                        };
                    }
                    else
                    {
                        wheelsetPivots[i].posRot = graph.GetPositionRotation(PivotLocations[i], accuracy);
                    }
                    _truckGizmoPosRots.Add(wheelsetPivots[i].posRot);
                }
                ...
                if (MainPivot.IsWheelset)
                {
                    MainPivot.GetComponentInChildren<Wheelset>()?.Roll(distance, velocity);
                }
                ...
                foreach (ArticulatedPivot pivot in Pivots)
                {
                    ...
                    pivot.posRot = TransformForDerailment(pivot.posRot, pivot.posRot.Position, wheelBoundsF);
            ...
            internal void PositionCoupler(LogicalEnd logicalEnd)
            {
                ...
                if (!(coupler == null))
                {
                    ...
                    float y = (Quaternion.Inverse(parentTransform.rotation) * Quaternion.LookRotation(lookDir)).eulerAngles.y;
                    coupler.transform.SetLocalPositionAndRotation(new Vector3(0f, couplerHeight, OffsetToEnd(thisEnd, extra)), Quaternion.Euler(0, y, 0));
                }
            }
            ...
            internal (Vector3 position, Quaternion rotation) GetMoverTargetPositionRotation()
            {
                ...
                if (this.CoupledTo(LogicalEnd.A) && !this.CoupledTo(LogicalEnd.B))
                {
                    (this.FrontIsA ? Pivots.Last(p => !p.IsWheelset) : Pivots.First(p => !p.IsWheelset)).mover.UpdateMovement(out goalPosition, out goalRotation, Time.deltaTime);
                }
                else if (this.CoupledTo(LogicalEnd.B) && !this.CoupledTo(LogicalEnd.A))
                {
                    (this.FrontIsA ? Pivots.First(p => !p.IsWheelset) : Pivots.Last(p => !p.IsWheelset)).mover.UpdateMovement(out goalPosition, out goalRotation, Time.deltaTime);
                }
                else
                {
                    MainPivot.mover.UpdateMovement(out goalPosition, out goalRotation, Time.deltaTime);
                }
                return (goalPosition, goalRotation);
            }
