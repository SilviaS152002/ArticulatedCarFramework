    ...
    namespace ArticulatedCarFramework
    {
        ...
        public partial class ArticulatedCar : Car
        {
            ...
            public (Graph.PositionRotation, float)[] GetRectCheck(Graph graph)
            {
                ...
                (Graph.PositionRotation, float)[] result = new (Graph.PositionRotation, float)[pos.Length - 1];
                for (int i = 0; i < result.Length; i++)
                {
                    result[i] = (
                        new Graph.PositionRotation()
                        {
                            Position = (pos[i + 1] + pos[i]) / 2f,
                            Rotation = Quaternion.LookRotation(Vector3.Scale(pos[i + 1] - pos[i], new Vector3(1, 0, 1)))
                        },
                        (pos[i + 1] - pos[i]).magnitude
                    );
                }
                return result;
            }
