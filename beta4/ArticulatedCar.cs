...
namespace NS15
{
    namespace ArticulatedCarFramework
    {
        ...
        public class ArticulatedPivotDefinition
        {
            ...
            public string TruckIdentifier;

            public string PivotA;
            public float OffsetA;

            public string PivotB;
        ...
        public class ArticulatedCarDefinition : CarDefinition
        {
            ...
            public new string TruckIdentifier = null;
            // public float LengthOffset;
            public Dictionary<string, ArticulatedPivotDefinition> Pivots;
            public string EndGearPivotF = null;
            public string EndGearPivotR = null;
        }
        ...
        public partial class ArticulatedCar : Car
        {
            ...
            protected override void FinishSetup()
            {
                ...
                foreach (KeyValuePair<string, ArticulatedPivotDefinition> namedPivotDef in Definition.Pivots)
                {
                    GameObject pivotObject = new GameObject(namedPivotDef.Key);
                    pivotObject.transform.SetParent(this.transform);
                    ArticulatedPivot pivot = pivotObject.AddComponent<ArticulatedPivot>();
                    pivot.Definition = namedPivotDef.Value;
                    pivot.mover.DebugId = $"Pivot {this}:{namedPivotDef.Key}";
            ...
            internal async void LoadModelsAsync()
            {
                try
                {
                    ...
                    foreach (ArticulatedPivot pivot in Pivots)
                    {
                        if (!string.IsNullOrEmpty(pivot.Definition.ModelIdentifier) && !_truckPrefabLoadTasks.TryGetValue(pivot.Definition.ModelIdentifier, out _))
                        {
            ...
            internal void HandleModelsLoaded()
            {
                ...
                MakeMaterialsUnique(this.gameObject, _bodyRenderers);
            ...
            protected override void DidLoadModels()
            {
                ...
                _rollingPlayer = bodyObject.AddComponent<RollingPlayer>();
                _rollingPlayer.profile = _setupPrefabs.RollingProfile;

                this.gameObject.AddComponent<CarPickable>().car = this;

                CarColorController carColorController = this.gameObject.AddComponent<CarColorController>();
            ...
            // Changes to HandleModelsLoaded should mean this is not needed
            private void PostSetupBody()
            {
                /*
                Transform mainBodyTransform = this.Resolve(MainPivot.BodyTransform);
                ...
                GameObject bodyTransformObject = BodyTransform.gameObject;

                MainPivot.mover.ClearBody();
                MainPivot.mover.ConfigureForBody(mainBodyTransform.gameObject);
                */
                Destroy(BodyTransform.gameObject.GetComponent<CarPickable>());
                this.gameObject.AddComponent<CarPickable>().car = this;
            }
            ...
            internal void SetupComponent(Model.Definition.Component component, ComponentSetup.Context setupContext, ComponentLifetime lifetime)
            {
                Transform parent = lifetime switch
                {
                    ...
                    ComponentLifetime.Model => base.transform.ResolveTransform(component.Parent, defaultReturnsReceiver: true),
            ...
            public override Location PositionWheelBoundsFront(Location wheelBoundsF, Graph graph, MovementInfo info, bool update)
            {
                ...
                foreach (ArticulatedPivot pivot in Pivots)
                {
                    ...
                    pivot.mover.Move(WorldTransformer.GameToWorld(pivot.posRot.Position), pivot.posRot.Rotation, isFirstPosition);
                }
            ...
            internal void PositionCouplerOld(LogicalEnd logicalEnd)
            {
                ...
                if (!(coupler == null))
                {
                    ...
                    coupler.transform.SetLocalPositionAndRotation(quaternion * (vector - parentTransform.position), quaternion * Unity.Mathematics.quaternion.LookRotation(vector3, vector2));
                }
